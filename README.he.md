...
**Windows 10 / 11 · ללא התקנה**

| | אפליקציה גרפית (אשף) | שורת פקודה (CLI) |
|:--|:--|:--|
| הורדה | [⬇ גרסה אחרונה](https://github.com/itielbru/gphotos-takeout-organizer/releases/latest) | [⬇ גרסה אחרונה](https://github.com/itielbru/gphotos-takeout-organizer/releases/latest) |
| הקובץ שצריך | `GPhotosTakeout-App-<version>-win-x64.zip` | `gptakeout-<version>-win-x64.exe` |

</div>

שני הקבצים נמצאים באותו עמוד [הגרסה האחרונה](https://github.com/itielbru/gphotos-takeout-organizer/releases/latest) — תבחרו את זה שמתאים לכם. ה-CLI הוא קובץ יחיד עצמאי (בלי ZIP). ה-App מגיע כ-ZIP (מגבלה של WinUI 3 — פירוט למטה) — חלצו אותו, התיקייה עצמאית מעבר לכך.

- **App** (ממשק גרפי): חלצו את `GPhotosTakeout-App-…-win-x64.zip`, ואז הריצו את `GPhotosTakeout.App.exe` שבתוך התיקייה שחולצה. בהרצה הראשונה לחצו על **התקן את ExifTool** כדי לאפשר כתיבת מטא-דאטה (הורדה חד-פעמית של ~10MB). כל שאר היכולות עובדות גם בלעדיו. לאחר מכן: הוסיפו את קובצי ה-Takeout → בחרו אפשרויות → הריצו.
- **CLI** (אוטומציה/סקריפטים): הריצו את `gptakeout-…-win-x64.exe` משורת הפקודה — ראו בהמשך.
...
>
> **למה ה-App הוא ZIP ולא EXE יחיד:** מצב `PublishSingleFile` של WinUI 3 לאפליקציות
...
> — באג ידוע ופתוח ב-Microsoft, לא משהו שאפשר לתקן ברמת הריפו הזה. ל-CLI אין תלות ב-XAML/WindowsAppSDK, אז EXE יחיד עובד בשבילו ללא בעיה.
