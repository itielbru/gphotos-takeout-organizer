...
הכפתור למעלה מוריד את קובץ ההתקנה ישירות. הריצו אותו, לחצו הלאה, וזהו — הוא מתקין לחשבון המשתמש (בלי הרשאות אדמין), מוסיף קיצור לתפריט התחלה, כולל גם את ה-CLI, ורושם מסיר-התקנה תקני. בהרצה הראשונה לחצו על **התקן את ExifTool** כדי לאפשר כתיבת מטא-דאטה (הורדה חד-פעמית של ~10MB); כל שאר היכולות עובדות גם בלעדיו. לאחר מכן: הוסיפו את קובצי ה-Takeout → בחרו אפשרויות → הריצו.
