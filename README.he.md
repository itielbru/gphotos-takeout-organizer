...
[![הורדת קובץ ההתקנה](https://img.shields.io/badge/%E2%AC%87%EF%B8%8F%20%D7%94%D7%95%D7%A8%D7%93%D7%94%20%D7%9C%D7%95%D7%95%D7%99%D7%A0%D7%93%D7%95%D7%A1-Setup%20EXE-0D7377?style=for-the-badge&logo=windows)](https://github.com/itielbru/gphotos-takeout-organizer/releases/latest/download/GPhotosTakeout-Setup-win-x64.exe)

**Windows 10 / 11 · קליק אחד · בלי הרשאות אדמין** · [![גרסה אחרונה](https://img.shields.io/github/v/release/itielbru/gphotos-takeout-organizer?label=%D7%92%D7%A8%D7%A1%D7%94&color=0D7377)](https://github.com/itielbru/gphotos-takeout-organizer/releases/latest)

</div>

הכפתור למעלה מוריד את קובץ ההתקנה ישירות. הריצו אותו, לחצו הלאה, וזהו — הוא מתקין לחשבון המשתמש (בלי הרשאות אדמין), מוסיף קיצור לתפריט התחלה, כולל גם את ה-CLI **וגם ExifTool מובנה**, ורושם מסיר-התקנה תקני. הכל עובד מהקופסה: הוסיפו את קובצי ה-Takeout → בחרו אפשרויות → הריצו.

מעדיפים בלי התקנה בכלל? שתי החלופות נמצאות בעמוד [הגרסה האחרונה](https://github.com/itielbru/gphotos-takeout-organizer/releases/latest):

| חלופה | הקובץ שצריך | הערות |
|:--|:--|:--|
| אפליקציה ניידת (ZIP) | `GPhotosTakeout-App-<version>-win-x64.zip` | חלצו והריצו את `GPhotosTakeout.App.exe` שבפנים. אותה אפליקציה, בלי קיצורים ומסיר-התקנה. |
| שורת פקודה (CLI) | `gptakeout-<version>-win-x64.exe` | קובץ יחיד עצמאי — ראו סעיף שורת הפקודה בהמשך. |
...
הגרסאות להורדה (קובץ ההתקנה וה-ZIP הנייד) כוללות בנייה מוצמדת ומאומתת-checksum של
[ExifTool](https://exiftool.org) בתיקיית `Tools\` ליד האפליקציה, כך שכתיבת מטא-דאטה עובדת בלי שום
הגדרה נוספת; גם ה-CLI מוצא אותה שם, או מקבל `--exiftool <path>` מפורש. ההתקנה בלחיצה אחת מתוך
האפליקציה (לתוך `%LocalAppData%\GPhotosTakeout\Tools`) נשארת למי שבונה מהמקור ולעדכון ExifTool
בעתיד. ללא ExifTool, האפליקציה עדיין מארגנת ומתארכת קבצים, אבל לא תכתוב metadata לתוכם.
